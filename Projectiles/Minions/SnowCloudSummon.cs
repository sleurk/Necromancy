...
namespace Necromancy.Projectiles.Minions
{
    public class SnowCloudSummon : ModProjectile
    {
        ...
        {
            Lighting.AddLight((int)(projectile.Center.X / 16f), (int)(projectile.Center.Y / 16f), 0f, 0.6f, 0.6f);
        }
            ...
            { //not returning to player
                ...
                { // has target
                    ...
                    { // shoot ready
                        ...
                        {
                            Vector2 shootVel = Vector2.UnitY * shootSpeed + projectile.velocity;

                            Vector2 pos = new Vector2(Main.rand.NextFloat(projectile.position.X + projectile.width * 1 / 4f, projectile.position.X + projectile.width * 3 / 4f), projectile.position.Y + projectile.height);

                            Dust.NewDust(pos, 2, 2, 80, shootVel.X, shootVel.Y);

                            int proj = Projectile.NewProjectile(pos.X, pos.Y, shootVel.X, shootVel.Y, mod.ProjectileType("SummonSnow"), projectile.damage / 2, projectile.knockBack, Main.myPlayer, 0f, 0f);
